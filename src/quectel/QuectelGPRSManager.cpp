...
QuectelGPRSManager::QuectelGPRSManager(GSMModemManager *gsmManager):GPRSManager(gsmManager)
{
...
bool QuectelGPRSManager::ConnectInternal()
{
    ...
    // Set apn
    gsmManager->ForceCommand(new ByteChar2ModemCMD(1, "IP", apn, QUECTEL_DEFINE_PDP));
    return false;
}
...
bool QuectelGPRSManager::OnGSMResponse(BaseModemCMD *request, char * response, size_t respLen, MODEM_RESPONSE_TYPE type)
{
    if (strcmp(request->cmd, QUECTEL_RESOLVE_DNS_CMD) == 0) {
        ...
        if (type == MODEM_RESPONSE_DATA) {
            ...
            char *cdnsgip = response + strlen(QUECTEL_RESOLVE_DNS_CMD) + 2;
            char *cdnsgipArgs[3];
    ...
    if (strcmp(request->cmd, QUECTEL_DEFINE_PDP) == 0) {
        if (type == MODEM_RESPONSE_OK) {
            ...
            if (login != NULL && login[0] != 0 && password != NULL && password[0] != 0) {
                ...
                gsmManager->ForceCommand(new Byte2Char2ModemCMD(1, 1, password, login, QUECTEL_AUTH_PDP, 9000000ul));
            } else if (password != NULL && password[0] != 0) {
                // password only
                // AT+CGAUTH=1,2,"PASSWORD"
                gsmManager->ForceCommand(new Byte2CharModemCMD(1, 2, password, QUECTEL_AUTH_PDP, 9000000ul));
            } else {
                // no auth
                // AT+CGAUTH=1,0
                gsmManager->ForceCommand(new Byte2ModemCMD(1, 0, QUECTEL_AUTH_PDP, 9000000ul));
            }
    ...
    if (strcmp(request->cmd, QUECTEL_AUTH_PDP) == 0) {
        if (type == MODEM_RESPONSE_OK) {
            // TODO: Activate
            gsmManager->ForceCommand(new Byte2ModemCMD(1, 1, QUECTEL_ACTIVATE_PDP, 9000000ul));
        } else if (type > MODEM_RESPONSE_OK) {
            ForceDeactivate();
        }
        return true;
    ...
    if (strcmp(request->cmd, QUECTEL_ACTIVATE_PDP) == 0) {
        if (type == MODEM_RESPONSE_OK) {
            ...
            gsmManager->ForceCommand(new ByteModemCMD(1, QUECTEL_GET_PDP_IP, 9000000ul));
        } else if (type > MODEM_RESPONSE_OK) {
            ForceDeactivate();
        }
        return true;
    }
    if (strcmp(request->cmd, QUECTEL_GET_PDP_IP) == 0) {
        if (type == MODEM_RESPONSE_DATA) {
            char *cgpaddr = response + strlen(QUECTEL_GET_PDP_IP) + 2;
            char *cgpaddrArgs[2];
...
bool QuectelGPRSManager::OnGSMEvent(char * data, size_t dataLen)
{
    ...
    if (IsEvent(GSM_QUECTEL_NETWORK_CLOSE_EVENT, data, dataLen)) {
        ForceDeactivate();
        return true;
    }
...
{
    return gsmManager->AddCommand(new IPResolveModemCMD(hostName, QUECTEL_RESOLVE_DNS_CMD, APN_CONNECT_CMD_TIMEOUT));
}